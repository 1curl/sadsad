                    ...
                    if res_table.update_id ~= updateid then
                        ...
                        if message_from_user then
                            ...
                            if text:match('^/start') then
                                sendTelegramNotification('Ïðèâåòñòâóþ! Òóò òû ìîæåøü óïðàâëÿòü ñâîèì áîòîì ñ ïîìîùþ êíîïîê ñíèçó. \ndeveloped by fokich')
                            elseif text:match('^Mode') then
                                mode = not mode
                                sendTelegramNotification(mode and 'Áîò âêëþ÷åí' or 'Áîò âûêëþ÷åí')
                            elseif text:match('^Stats') then
                                local isl = isLien and 'ëüíà: ' or 'õëîïêà: '
                                sendTelegramNotification('Êîëè÷åñòâî ñîáðàííîãî '.. isl .. totalct .. '\nÎñòàëîñü: ' .. SliderInt[0] - totalct)
...
function main()
    ...
    while true do
                ...
                if res then         
                    ...
                    if result then
                        ...
                        if totalct ~= SliderInt[0] then
                            ...
                            mode = false
                            sampAddChatMessage('{cc66ff}[auto-cotton]: {FFFFFF}Áîò ñîáðàë çàäàííîå êîë-âî ðåñóðñîâ!', -1)
                            sendTelegramNotification('Áîò ñîáðàë çàäàííîå êîë-âî ðåñóðñîâ')
                        end
                    else
                        renderFontDrawText(fontonscreen, 'Êóñò íå íàéäåí', resx / 2 + 500, resy / 2, -1, false)
                    end
                else
                    renderFontDrawText(fontonscreen, 'Êóñò íå íàéäåí', resx / 2 + 500, resy / 2, -1, false)
                end
        ...
        -- Îáðàáîòêà êîìàíäû ÷åðåç cheat
        if testCheat('acccc') then
            mode = not mode
        end
        
        -- Îáðàáîòêà íàæàòèÿ êëàâèøè END
        if isKeyDown(VK_END) then
            -- Ïåðåêëþ÷àåì ñîñòîÿíèå ðåæèìà (mode)
            mode = not mode
            local modeText = mode and 'âêëþ÷åí' or 'âûêëþ÷åí'
            sampAddChatMessage('{cc66ff}[auto-cotton]: {FFFFFF}Áîò ' .. modeText, -1)
            wait(200)  -- Äîáàâëÿåì çàäåðæêó, ÷òîáû èçáåæàòü ìíîæåñòâåííûõ ñðàáàòûâàíèé íà îäíî íàæàòèå
        end
    end
end
...
function lookFor3dText()
    for i = 0, 2048 do
        if sampIs3dTextDefined(i) then
            ...
            if text:find('Ìîæíî ñîáðàòü') and text:find('10') then
                if text:find("Ë¸í") and isLien then
                    table.insert(array, {['position'] = {x, y, z}, ['distance'] = distance, ['type'] = 2})
                elseif text:find("Õëîïîê") and not isLien then
                    table.insert(array, {['position'] = {x, y, z}, ['distance'] = distance, ['type'] = 2})
                end
            else
                if text:find('ýòàï 2') and text:find('Îñòàëîñü') then
                    if text:find("Ë¸í") and isLien then
                        for i = 1, 59 do
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
    if stopWithDialog then
        mode = false
        sendTelegramNotification('Àäìèíèñòðàòîð îâòåòèë âàì â /pm: ' .. text)
        if quitgame then
...
function sampev.onSetPlayerPos(position)
    if stopWithSetPlayerPos then
        mode = false
        sendTelegramNotification('Ñåðâåð/Àäìèíèñòðàòîð èçìåíèë âàøó ïîçèöèþ: ' .. position)
        if quitgame then
            os.execute('taskkill /IM gta_sa.exe /F')
        end
    end
...
function sampev.onServerMessage(color, text)
    if stopWithChatMessage and text:find("Âû òóò?") or text:find("Âû òóò") or text:find("Âû çäåñü") or text:find("Âû çäåñü?") or text:find("âû òóò?") or text:find("âû òóò") or text:find("âû çäåñü") or text:find("âû çäåñü?") or text:find("â û ç ä å ñ ü?") then
        mode = false
        local allchat = '\n'
        for i = 100-3, 99 do
        ...
        sendTelegramNotification('Áîòà ñïðîñèëè: âû òóò?'  .. allchat)
        if quitgame then
            os.execute('taskkill /IM gta_sa.exe /F')
        end
    end
...
imgui.OnFrame(function() return WinState[0] end, function(player)
    ...
    if imgui.BeginChild(u8'Íàñòðîéêè', imgui.ImVec2(190, 160), true) then
        imgui.PushFont(big)
        if imgui.RadioButtonBool(u8'Ñòàòóñ', mode) then mode = not mode end
        if imgui.RadioButtonBool(u8'Áåã', sprint) then sprint = not sprint end
        if imgui.RadioButtonBool(u8'Ïðûæêè', autojump) then autojump = not autojump end
        local isl = isLien and u8'ë¸í' or u8'õëîïîê'
        if imgui.RadioButtonBool(u8'Ñîáèðàþ ' .. isl, isLien) then
            isLien = not isLien 
        end
        imgui.SliderInt('##', SliderInt, 1, 10000)
        imgui.SetCursorPos(imgui.ImVec2(135, 122))
        if imgui.Button(u8'Ñáðîñ') then totalct = 0 end
...
imgui.OnFrame(function() return WinSet[0] end, function(player)
    ...
    imgui.PushFont(big)
    imgui.Text(u8'Âûêëþ÷åíèå')
    if imgui.RadioButtonBool(u8'ïðè äèàëîãå îò àäìèíà', stopWithDialog) then stopWithDialog = not stopWithDialog end
    if imgui.RadioButtonBool(u8'ïðè èçìåíåíèè ïîçèöèè ñåðâåðîì', stopWithSetPlayerPos) then stopWithSetPlayerPos = not stopWithSetPlayerPos end
    if imgui.RadioButtonBool(u8'ïðè ïðîâåðêå íà áîòà â ÷àòå', stopWithChatMessage) then stopWithChatMessage = not stopWithChatMessage end
    if imgui.RadioButtonBool(u8'Âûõîäèòü èç èãðû', quitgame) then quitgame = not quitgame end
    imgui.End()
end)
...
imgui.OnFrame(function() return WinTg[0] end, function(player)
    ...
    imgui.PushFont(big)
    imgui.InputText(u8"Òîêåí", tknField, 256)
    ...
    if imgui.Button(u8'Òåñòîâîå Ñîîáùåíèå') then
        sampAddChatMessage('[Telegram] Îòïðàâëÿþ òåñòîâîå ñîîáùåíèå',-1)
        sendTelegramNotification('Òåñòîâîå ñîîáùåíèå îò '..sampGetPlayerNickname(select(2, sampGetPlayerIdByCharHandle(PLAYER_PED))))
    end
    imgui.End()
end)
...
imgui.OnFrame(function() return WinStats[0] end, function(player)
    ...
    imgui.Begin('##Window', WinTg, imgui.WindowFlags.NoResize)
    imgui.PushFont(big)
    local isl = isLien and u8'ëüíà: ' or u8'õëîïêà: '
    imgui.Text(u8'Êîë-âî ' .. isl .. totalct)
    imgui.Text(u8'Îñòàëîñü: ' .. SliderInt[0] - totalct)
    imgui.End()
end).HideCursor = true
